...
<script>
    ...
    layui.use(['form','layer','laydate','table'], function(){
        ...
        table.render({
            elem: '#demo'
            // ,height: 700
            ,url: '<%=path%>/employeePermission/paging' //数据接口
            ,method:"post"
...
//                    html+='<label class="layui-form-label">父级权限</label>';
//                    html+='<div class="layui-input-block">';
//                    html+='<select id="parentId" lay-search>';
//                    html+='<option value="0">0</option>';
//                    var pdata={};
                    <%--pdata.parentId='${parentId}';--%>
                    <%--$.ajaxSettings.async=false;--%>
                    <%--$.post('<%=path%>/employeePermission/allParent',pdata,function(res){--%>
                        <%--var list=res.data;--%>
        ...
        table.on('tool(demo)', function(obj){
            ...
            if (obj.event === 'detail') {
                popUpBox = layer.open({
                    ...
                    maxmin: true,
                    title: data.name,
                    content:'<%=path%>/employeePermission/jumpDetail?id='+data.id
                })
            }
            // 通过
            if (obj.event === 'update') {
...
//                    maxmin: true,
            ...
            if (obj.event === 'delete') {
                layer.confirm("确定删除吗?",function(resdata){
                    $.post('<%=path%>/employeePermission/delete',{id:data.id},
                        function (res) {
                            if (res.code==0){
                                refreshTable(res.msg);
                            }else if(res.code==-1){
...
//                    maxmin: true,
        ...
        form.on('submit(addFormDemo)',function(){
            ...
            console.log(data);
            $.post("<%=path%>/employeePermission/add",data,function(res){
                if(res.code==0){
                    refreshTable(res.msg);
                }else if(res.code==-1){
                    layer.msg(res.msg);
...
//                    maxmin: true,
        ...
        form.on('submit(updateFormDemo)',function(){
            ...
            data.name=$("#name").val();
            data.descript=$("#descript").val();
            $.post("<%=path%>/employeePermission/update",data,function(res){
                if(res.code==0){
                    refreshTable(res.msg);
                }else if(res.code==-1){
                    layer.msg(res.msg);
