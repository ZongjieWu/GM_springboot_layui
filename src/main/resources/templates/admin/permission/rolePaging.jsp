        ...
        <div class="layui-form layui-card-header layuiadmin-card-header-auto">
            <div class="layui-form-item">
                ...
                <div class="toRight">
                    ...
                    <%--</div>--%>
                    <div class="layui-inline">
                        <input type="text" id="keywords" name="keywords" placeholder="请输入职位名称" autocomplete="off" class="layui-input" style="width: 400px;">
                    </div>
                    <div class="layui-inline">
...
<script>
    ...
    layui.use(['form','layer','laydate','table'], function(){
        ...
        table.render({
            elem: '#demo'
            // ,height: 700
            ,url: '<%=path%>/employeeRole/paging' //数据接口
            ,method:"post"
            ,where: {}
            ,page: true //开启分页
            ,limit:10
...
//                    maxmin: true,
                ...
                popUpBox=layer.open({
                    ...
                    area: ['660px', '520px'], //宽高
                    content: '<%=path%>/employeePermission/jumpSetPermission?id='+data.id
                });
            }
        };
        //监听工具条
...
//                    maxmin: true,
            ...
            if (obj.event === 'delete') {
                layer.confirm("确定删除吗?",function(resdata){
                    $.post('<%=path%>/employeeRole/delete',{id:data.id},
                        function (res) {
                            if (res.code==0){
                                refreshTable(res.msg);
                            }else {
        ...
        form.on('submit(addFormDemo)',function(){
            ...
            $.post("<%=path%>/employeeRole/add",data,function(res){
                if(res.code==0){
                    refreshTable(res.msg);
                }else if(res.code==-1){
                    layer.msg(res.msg);
...
//                    maxmin: true,
        ...
        form.on('submit(updateFormDemo)',function(){
            ...
            $.post("<%=path%>/employeeRole/update",data,function(res){
                if(res.code==0){
                    refreshTable(res.msg);
                }else if(res.code==-1){
                    layer.msg(res.msg);
